...
class CryptoFetcher:
    ...
    def get_history(self, symbol: str, interval: str = '1d', limit: int = 100) -> List[Dict]:
        """
        获取历史 K 线数据
        ...
        try:
            ...
            logger.error(f"Get history failed for {symbol}: {e}")
        return []
...
if __name__ == '__main__':
    ...
    print(f"BTC Price: ${data['price'] if data else 'N/A':,.2f}")
    print(f"24h Change: {data['change_24h'] if data else 'N/A':+.2f}%")
    
    # 测试涨幅榜
    print("\nTop Gainers:")
    gainers = fetcher.get_top_gainers(5)
    for g in gainers:
        print(f"  {g['symbol']}: ${g['price']:,.2f} ({g['change_24h']:+.2f}%)")
    
    # 测试历史数据
    print("\nBTC History (last 5 candles):")
    history = fetcher.get_history('BTC', '1d', 5)
    for candle in history:
        print(f"  {datetime.fromtimestamp(candle['timestamp']/1000).strftime('%Y-%m-%d')}: ${candle['close']:,.2f}")
