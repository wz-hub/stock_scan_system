const https = require('https');
const fs = require('fs');

// 飞书开放平台配置
const APP_ID = 'cli_a9eddbc73ab85bcd';
const APP_SECRET = 'pSul1MqDerYxg6dI69OktfKaxpVAa7qI';
const DOC_ID = 'Kekcdlaypo4wwrxQjuccRFk6nyh';
// 从之前的响应知道，文档的根块 ID 就是文档 ID 本身
const ROOT_BLOCK_ID = 'Kekcdlaypo4wwrxQjuccRFk6nyh';

// 读取文档内容
const docContent = fs.readFileSync('/home/wz/.openclaw/workspace/ml_trading/DOC_CONTENT.md', 'utf8');

// 获取 tenant_access_token
function getTenantAccessToken() {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({
      app_id: APP_ID,
      app_secret: APP_SECRET
    });

    const options = {
      hostname: 'open.feishu.cn',
      port: 443,
      path: '/open-apis/auth/v3/tenant_access_token/internal',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Content-Length': data.length
      }
    };

    const req = https.request(options, (res) => {
      let responseData = '';
      res.on('data', (chunk) => responseData += chunk);
      res.on('end', () => {
        try {
          const result = JSON.parse(responseData);
          if (result.code === 0) {
            resolve(result.tenant_access_token);
          } else {
            reject(new Error(`获取 token 失败：${JSON.stringify(result)}`));
          }
        } catch (e) {
          reject(new Error(`解析失败：${responseData}`));
        }
      });
    });

    req.on('error', reject);
    req.write(data);
    req.end();
  });
}

// 删除文档块
function deleteDocumentBlocks(token, docId, parentBlockId) {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: 'open.feishu.cn',
      port: 443,
      path: `/open-apis/docx/v1/documents/${docId}/blocks/${parentBlockId}/children`,
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`
      }
    };

    const req = https.request(options, (res) => {
      let responseData = '';
      res.on('data', (chunk) => responseData += chunk);
      res.on('end', () => {
        try {
          const result = JSON.parse(responseData);
          resolve({ statusCode: res.statusCode, data: result });
        } catch (e) {
          resolve({ statusCode: res.statusCode, rawData: responseData });
        }
      });
    });

    req.on('error', reject);
    req.end();
  });
}

// 创建文档块（子块）
function createChildBlocks(token, docId, parentBlockId, blocks) {
  return new Promise((resolve, reject) => {
    const data = JSON.stringify({
      blocks: blocks
    });

    const options = {
      hostname: 'open.feishu.cn',
      port: 443,
      path: `/open-apis/docx/v1/documents/${docId}/blocks/${parentBlockId}/children`,
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Content-Length': Buffer.byteLength(data),
        'Authorization': `Bearer ${token}`
      }
    };

    const req = https.request(options, (res) => {
      let responseData = '';
      res.on('data', (chunk) => responseData += chunk);
      res.on('end', () => {
        try {
          const result = JSON.parse(responseData);
          resolve({ statusCode: res.statusCode, data: result });
        } catch (e) {
          resolve({ statusCode: res.statusCode, rawData: responseData });
        }
      });
    });

    req.on('error', reject);
    req.write(data);
    req.end();
  });
}

// 将 Markdown 转换为飞书文档块（简化版）
function markdownToBlocks(markdown) {
  const lines = markdown.split('\n');
  const blocks = [];
  
  for (const line of lines) {
    if (line.startsWith('# ')) {
      blocks.push({
        block_type: 1,
        heading1: {
          elements: [{
            text_run: { content: line.substring(2) + '\n' }
          }]
        }
      });
    } else if (line.startsWith('## ')) {
      blocks.push({
        block_type: 2,
        heading2: {
          elements: [{
            text_run: { content: line.substring(3) + '\n' }
          }]
        }
      });
    } else if (line.startsWith('### ')) {
      blocks.push({
        block_type: 3,
        heading3: {
          elements: [{
            text_run: { content: line.substring(4) + '\n' }
          }]
        }
      });
    } else if (line.startsWith('#### ')) {
      blocks.push({
        block_type: 4,
        heading4: {
          elements: [{
            text_run: { content: line.substring(5) + '\n' }
          }]
        }
      });
    } else if (line.startsWith('- ')) {
      blocks.push({
        block_type: 11,
        bullet: {
          elements: [{
            text_run: { content: line.substring(2) + '\n' }
          }]
        }
      });
    } else if (line.trim() === '') {
      blocks.push({
        block_type: 5,
        text: {
          elements: [{
            text_run: { content: '\n' }
          }]
        }
      });
    } else {
      blocks.push({
        block_type: 5,
        text: {
          elements: [{
            text_run: { content: line + '\n' }
          }]
        }
      });
    }
  }
  
  return blocks;
}

async function main() {
  try {
    console.log('📝 开始更新飞书文档...');
    console.log('文档 ID:', DOC_ID);
    console.log('根块 ID:', ROOT_BLOCK_ID);
    console.log('内容大小:', docContent.length, '字符');
    
    // 获取 token
    console.log('\n🔑 正在获取 tenant_access_token...');
    const token = await getTenantAccessToken();
    console.log('✅ Token 获取成功');
    
    // 删除现有子块（清空文档内容）
    console.log('\n🗑️  正在清空现有内容...');
    const deleteResult = await deleteDocumentBlocks(token, DOC_ID, ROOT_BLOCK_ID);
    console.log('📊 删除响应状态:', deleteResult.statusCode);
    if (deleteResult.data) {
      console.log('📄 删除结果:', JSON.stringify(deleteResult.data, null, 2));
    }
    
    // 将 Markdown 转换为块
    console.log('\n🔄 正在转换 Markdown 为文档块...');
    const blocks = markdownToBlocks(docContent);
    console.log('✅ 转换完成，共', blocks.length, '个块');
    
    // 分批创建块（每次最多 50 个）
    console.log('\n📤 正在创建文档块...');
    const batchSize = 50;
    let successCount = 0;
    
    for (let i = 0; i < blocks.length; i += batchSize) {
      const batch = blocks.slice(i, i + batchSize);
      const batchNum = Math.floor(i / batchSize) + 1;
      const totalBatches = Math.ceil(blocks.length / batchSize);
      console.log(`  批次 ${batchNum}/${totalBatches}: 创建 ${batch.length} 个块...`);
      
      const createResult = await createChildBlocks(token, DOC_ID, ROOT_BLOCK_ID, batch);
      console.log(`  📊 响应状态:`, createResult.statusCode);
      
      if (createResult.statusCode === 200 && createResult.data?.code === 0) {
        successCount += batch.length;
        console.log(`  ✅ 成功`);
      } else {
        console.log(`  ❌ 失败:`, JSON.stringify(createResult.data || createResult.rawData, null, 2).substring(0, 200));
      }
      
      // 避免 API 限流
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    
    console.log('\n📊 总结:');
    console.log('  总块数:', blocks.length);
    console.log('  成功:', successCount);
    console.log('  失败:', blocks.length - successCount);
    
    console.log('\n✅ 文档更新完成!');
    console.log('doc_id:', DOC_ID);
    console.log('doc_url:', `https://feishu.cn/docx/${DOC_ID}`);
    
  } catch (error) {
    console.error('\n❌ 错误:', error.message);
    console.error(error.stack);
    process.exit(1);
  }
}

main();
